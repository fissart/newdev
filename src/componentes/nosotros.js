    ...
    <div key={number._id}>
      {number._id !== id ?
        ...
        <div style={{ backgroundColor: 'orange' }} className="card" key={number._id}>
          <button onClick={() => { update(number._id); setId() }} className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">Actualizar</button>
          <CKEditor
  ...
  return (
    <div className="contenedor">
      <div style={{ margin: 'auto', display: 'block', textAlign: 'right' }}>
        {localStorage.getItem("user")&&JSON.parse(localStorage.getItem("user")).rol=='1' ? <button onClick={() => wwwww()} className="items-center justify-center px-1 py-1 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">Crear</button> : <></>}
      </div>
      <div>{listItems}</div>
      <ToastContainer
